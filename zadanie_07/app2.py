...
strength_of_cards = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
...
if __name__ == '__main__':
    ...
    for card in cards:
        ...
        the_best = max(card[0], key=lambda x: card[0].count(x) if x !="J" else 0)
        #the_best = max(card[0], key=lambda x: card[0].count(x))
        print("max: ", the_best)
        for strength in strength_of_cards:
            if strength != "J":
                if strength == the_best:
                    number_of_sim_card[card[0].count(strength)+jokers].append(strength*card[0].count(strength)+"J"*jokers)
                else:
