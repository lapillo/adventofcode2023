...
if __name__ == '__main__': 
    ...
    for m,map in enumerate(maps):
        #find line with one diff
        print ("rozmiar: ", len(map))
        print("mirror: ", get_mirror_lines(map))
        for i in range(len(map)):
            for j in range(i+1,len(map)):
                if len( [x for x,y in zip(list(map[i]),list(map[j])) if x!=y] ) in[1] :
                    print ("mam w mapie -: ", m, " ", i, " ", map[i], " ", j, " ", map[j])
                    old = map[i]
                    map[i]=map[j]
                    if (get_mirror_lines(map) !=(0,0) ):
                        print ("znalazlem: ", i, " ", j, " wartosc: ", get_mirror_lines(map))
                    map[i]=old
                if len( [x for x,y in zip(list(map[i]),list(map[j])) if x!=y] ) in[0] :
                    print ("mam w mapie +: ", m, " ", i, " ", map[i], " ", j, " ", map[j])       
        
        
        #get current line
        # line = get_mirror_lines()
